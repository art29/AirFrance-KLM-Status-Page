...
class Status extends React.Component {
    ...
    constructor(props) {
        super(props);
        this.state = {results: [], airline: 'AF', flight_number: '', errorMessage: ''}; // Result and Flight Number states
    ...
    fetchFlights = (airline, flight_number, start_date, end_date) => {
        fetch("https://api.airfranceklm.com/opendata/flightstatus/?carrierCode=" + airline + "&flightNumber=" + flight_number + "&startRange=" + start_date + "&endRange=" + end_date, {
            headers: new Headers({
            ...
            .then(result => {
                if (result['operationalFlights'] !== undefined) {
    ...
    toggleButtonState = () => {
        ...
        this.fetchFlights(airline, flight_number, start_date.toISOString().split('.')[0] + "Z", end_date.toISOString().split('.')[0] + "Z")
    }
            ...
            <div>
                <div className="px-4 text-center">
                    <div className="clearfix">
                        <img src="./air-france-logo.svg" height="25" alt={"Air France Logo"}/>
                        <img src="./klm-logo.svg" height="50" alt={"KLM Logo"}/>
                    </div>
                    <br/>
                    <h1 className="display-5 fw-bold">Air France/KLM Status Page*</h1>
                    <p className="text-muted">* This website is not affiliated in any way with Air France/KLM or any
                        other entities</p>
                    <div className="col-lg-6 mx-auto">
                        <div className="input-group mb-3">
                            <select value={this.state.airline} onChange={this.handleAirlineUpdate}
                                    className="form-select" id="airline" style={{textAlign: 'right'}} dir={"rtl"}>
                                <option value="AF">AF/AFR</option>
                                <option value="KL">KL/KLM</option>
                            </select>
                            <input type="text" className="form-control" id="flight-number" placeholder="349"
                                   onChange={this.handleFlightNumberUpdate} value={this.state.flight_number}/>
                        </div>
                        <div className="d-grid gap-2 mt-1 d-sm-flex justify-content-sm-center">
                            <button type="button" className="btn btn-primary btn-lg px-4 gap-3"
                                    onClick={this.toggleButtonState}>Check Status
                            </button>
                        </div>
                        <br/>
                        <div>
